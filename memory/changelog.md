...
## 2026-09-26 · Claude (BUILD mode) · Fix: two concurrent appends broke both lab cards; tests independent of task pushes
- What happened: at 06:29 UTC the Brain applied Claude's queued card R4-BBRSI@1.0 to strategies_lab.yaml; the research run started at 06:28 appended the engine's variant card donchian_breakout-VEXIT@1.0 to its own (older) copy. Its `git pull --rebase` merged the two additions with `.gitattributes` `merge=union`, which merges line by line and wrote the lines both cards share (`targets:`, `- 2R`, `split:` ...) only once: the cards were interleaved (VEXIT got R4-BBRSI's edge block, R4-BBRSI lost its targets, time stop and weaknesses). The engine could not load either card, so neither was tested.
- Repair (operator PR): strategies_lab.yaml = the Brain's applied version (R4-BBRSI complete, identical to its queue record) + the variant card regenerated from the same research run's results (same evidence line as research.json). Neither card had been tested, so no registry record changes.
- Prevention: every append-only file (and events.yaml) now merges with the `append` driver (append_merge.py): both additions kept whole, one after the other; a changed or deleted earlier line is a conflict, never a silent merge. All four workflows register it before `git pull --rebase` and run `memory_guard.py --after-sync` before pushing (append-only files still start with the pushed main; every lab card still loads) - a broken merge is never pushed.
- Guard gap: a reference project record ([R1] ... [R8]) in memory/research_sources.md must name the full 40-character commit hash (the weekly research's [R1] re-check named "commit 484776c"; it stays - memory is append-only). tasks/weekly_research.md says how to find the full hash.
- Tests no longer depend on today's lab cards or memory records: the lab tests and every offline engine run use the lab file's header only (an empty lab); the reference-project test reads the build session's first record per project. A test fails whenever a card in the real strategies_lab.yaml cannot be loaded by the engine.

## 2026-09-26 · Claude (BUILD mode, operator request) · Email format v2 - type pill first, an ACTION box in every email, PAPER emails
- Design: the operator's canvas "Signal Agent Emails v2" (15 artboards). Kept `engine/mailkit.py` (blocks, colours, subject(), val(), plain()) and added the type pills, the PAPER colours (#1D4E89 on #E4EDF8, a 2 px dashed blue card - never the LIVE green), the score line, progress bar, squares and trade timeline blocks. Rebuilt every template in `engine/emails.py` (A LIVE signal, B LIVE update, C PAPER signal, D PAPER update, E PAPER complete, F ALERT, G FIXED, H briefing 08:20, I briefing 14:20 / 21:20, J daily, K weekly) and removed the old ones they replace: the ACTION NEEDED / RISK NOTICE / WATCH notices, the old [ENTRY] / [EXIT] / [DAILY] text templates in `engine/briefs.py` and the [WEEKLY] text email in `engine/digest.py` (its numbers stay; the fact sheet keeps the engine's daily block).
- Subjects start with the type (LIVE / PAPER / ! / ✓ Fixed / 08:20 / Daily / Week nn), under 70 characters. Every email has an ACTION box right under its banner ("None." is written out).
- New emails and data: PAPER emails for PAPER_TRADING versions (never a lab card) with the paper counter (n of 20) and the paper score vs the backtest; at most 3 PAPER emails an hour, the rest listed in the next briefing; PAPER complete once at the 20th closed paper signal (YOUR DECISION only when the approval rules pass - approval is still only the operator's config line). FILLED (first scan after the entry) and WARNING (a high-impact event within the hour while a trade is open; the agent never moves a stop). Every update is a reply in its entry email's thread (Message-ID / In-Reply-To / References).
- ALERT / FIXED: one layout and a registry per workflow (reports/alerts_scan.json, alerts_brain.json): one ALERT per problem, one FIXED when it recovers. New triggers: no scan for 2 hours and a Claude task 60 minutes late (the Brain workflow's watchdog, every 15 minutes), tests red on main (tests.yml, the first red run), market data unsafe on a SIGNAL coin, lost emails. Bad data on candidate coins and Binance futures 451 are only a SYSTEM HEALTH line.
- Briefings 14:20 / 21:20 show only what changed since the previous briefing (a snapshot is saved after each one); nothing changed = no email. The 08:20 briefing adds AGENT PLAN TODAY and STRATEGY PROGRESS (cells of memory/strategy_registry.csv). Daily and weekly: live / paper tiles, lifecycle changes, BUILT and the progress history from this changelog + merged PR titles, learned this week (fixes, lessons.md, research_sources.md), tests count.
- Unchanged: signals, fees, risk limits, gates, lifecycle and approvals. `signals.email_watching` was removed (WATCH setups stay in the reports).

## 2026-09-26 · Claude (BUILD mode, operator request) · Fix: a queued scan's reports clashed with the scan before it
- What happened: at 09:16 UTC the hourly scan was queued behind a manual scan (the TEST emails). GitHub started it from the main of the moment it was queued, so its generated reports (latest.md, positions.json, feeds.json ...) clashed with the manual scan's on `git pull --rebase`; the old `|| true` hid that, and the push then failed. Nothing was lost; the next scan started fresh.
- Fix: `git_sync.sh` (scan.yml and research.yml Save steps). When only files under reports/ clash, this run's copies win (every run rebuilds them). A clash anywhere else stops the run and nothing is pushed. `memory_guard.py --after-sync` still runs before every push. Tests: tests/test_sync.py GitSync (a real repository: add/add and content clashes, a clash outside reports/).
- Also fixed: the new watchdog's first run (09:24 UTC) sent an ALERT "Claude daily review missing" for 25 Sep and, one second later, its FIXED email, because 26 Sep's review had already arrived. The watchdog now raises no ALERT when a later report of the same task already exists (the task works again). Test: Alerts.test_watchdog_no_alert_when_a_newer_report_arrived.

## 2026-09-26 · Claude (BUILD mode, operator request) · Nov / Dec jobs report dates; the Claude tasks recreated for Full network access
- events.yaml: + US jobs report (Oct data) Fri 6 Nov 2026 08:30 New York = 13:30 UTC and (Nov data) Fri 4 Dec 2026 08:30 = 13:30 UTC (winter time from 1 Nov), read on the official BLS release calendar (www.bls.gov refuses cloud servers; the same pages are served at blsmon1.bls.gov) - check: official_page. The same calendar also shows CPI on 10 Nov and 10 Dec 08:30, as already listed (their `check` stays as it is: only the operator edits existing lines).
- The operator set the environment's network access to Full, which reaches new sessions fully: the briefing, daily review and weekly research tasks were recreated as new sessions (same prompts and times; the old schedules deleted). Their prompts add: try `curl` when WebFetch cannot open a page; BLS pages via blsmon1.bls.gov.
- Test fix: test_brain.CalendarFile used the live events.yaml to show a missing November NFP; it now uses its own calendar (and checks the real file has no NFP gap in Nov / Dec).
