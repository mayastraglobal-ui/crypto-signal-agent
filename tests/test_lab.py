...
class EndToEnd(unittest.TestCase):
    ...
    def test_lab_card_through_the_engine(self):
        ...
        # one trial per cell + one re-evaluation per cell under the family table (Phase 19 A)
        self.assertEqual(len(rows), 2 * len(res["cells"]))
        self.assertEqual(sum(r["origin"].startswith(T.REEVAL) for r in rows), len(res["cells"]))
