...
| `memory/playbook.md` | Per market regime: which strategies made / lost money in the backtests, and what "no trade" looks like (rewritten by every research run) | No, it's generated |
...
| `memory/curriculum.md` | Beginner lessons - one arrives with each [DAILY] email | Yes - edit or add lessons |
...
- **Edge block:** every strategy card says WHY it should make money - `edge: {who_pays, mechanism, fails_when,
  kill_rule}`. It is a hypothesis, not proof; the approval pack shows it at the top, next to the measured numbers.
  Lab cards without one are refused (control twins are exempt). It is not part of the rules (no new version needed).
- **`memory/market_mechanics.md`:** how markets move, each record with its evidence class. The build session's
  entries are marked unsourced CLAIMs; the weekly research confirms or rejects one per week with a real source.
- **`memory/playbook.md`:** generated by the daily research run from backtest trades split by the regime at entry.
  The briefing shows the part for the regimes the coins are in right now - including "nothing made money here:
  stand aside".
- **`memory/curriculum.md`:** 14 short lessons (R, stops, fees, overfitting, regimes, twins, edges, sizing, "no
  trade" ...), one per [DAILY] email, each pointing at something you can see in this system.
