...
| `memory/playbook.md` | Which strategy families work in BULL / BEAR / RANGE / TRANSITION on which timeframe, and per regime what made / lost money and what "no trade" looks like (rewritten every Sunday) | No, it's generated |
...
| `memory/beginner_course.md` | Beginner lessons for you - one each day, linked from the DAILY REVIEW email | Yes - edit or add lessons |
...
## Edge, mechanics, playbook, lessons (Phase 17 B)

- **Edge block:** every strategy card says WHY it should make money - `edge: {type (behavioural / forced_flow /
  risk_premium / structural), works_in (regimes), who_pays, mechanism, fails_when, kill_rule}`. It is a hypothesis, not proof; the approval pack shows it at the top, next to the measured numbers.
  Lab cards without one are refused (control twins are exempt). It is not part of the rules (no new version needed).
- **`memory/market_mechanics.md`:** how markets move, each record with its evidence class, source, mechanism and the
  strategies that use it. The build session's
  entries are marked unsourced CLAIMs; the weekly research confirms or rejects one per week with a real source.
- **`memory/playbook.md`:** generated every Sunday by the research run from backtest trades split by the regime at
  entry: a family x timeframe x BULL / BEAR / RANGE / TRANSITION table, then each regime in detail.
  The briefing shows the part for the regimes the coins are in right now - including "nothing made money here:
  stand aside".
- **`memory/curriculum.md`:** the agent's reading plan (24 items). The daily review studies one a day, writes one
