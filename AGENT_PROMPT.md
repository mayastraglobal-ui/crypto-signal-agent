...
- Only **APPROVED** versions send LIVE emails; PAPER_TRADING versions send PAPER (practice) emails.
...
**Email format v2 (operator-approved design: the canvas "Signal Agent Emails v2").** Every email is short and readable in about 30 seconds on a phone: an HTML card (600 px, table layout, inline CSS, no external fonts, no JavaScript, no images except the chart image on trade emails) plus a plain-text copy. Never raw markdown symbols. **The type pill comes first**: LIVE (solid green, real money, APPROVED only), PAPER (blue on light blue, a dashed blue card - never the LIVE green or the word LIVE), ALERT (solid red), FIXED (green on light green), BRIEFING / DAILY (teal on light teal), WEEKLY (solid teal). Subjects start with the type ("LIVE ...", "PAPER ...", "! ...", "✓ Fixed ...", "08:20 ...", "Daily ...", "Week nn ...") and stay under 70 characters. Order: header → banner / headline → **ACTION box, always** (DO NOW, PRACTICE NOW, ACTION or YOUR DECISION; "None." is valid) → numbers (plan table or tiles) → at most 3 reasons or 1 lesson → score line → 1-2 buttons → footer. All numbers come from the engine's files; a missing value is shown as "–", never guessed. Colour only helps: the words always say LONG / SHORT / WIN / LOSS. Idempotent IDs prevent duplicates.

- **A. LIVE signal** (APPROVED only): banner, DO NOW (the limit order and the stop), plan table (entry zone, stop −1R, targets in R with what to do), your size and the loss if stopped, the chart image, max 3 reasons, 4 check chips, score line (the strategy's live record), buttons: live chart, backtest chart.
- **B. LIVE update**: FILLED, TP1, TP2, rest stopped at entry, stop hit, time exit, exit rule, cancelled, WARNING (a high-impact event within the hour while the trade is open; the agent never moves a stop). Each is a **reply in the entry email's thread** (Message-ID / In-Reply-To / References). DO NOW, 4 tiles, TP1: the trade so far; stop hit: why it lost (the engine's failure tag) and "is the strategy still OK?" (live record, losing streak vs the backtest's worst). Score line: today, week, open trades.
- **C. PAPER signal / D. PAPER update** (PAPER_TRADING versions, never a lab card): practice only - PRACTICE NOW, no money boxes, ROAD TO REAL MONEY (paper signal n of 20, paper score vs the backtest). At most 3 PAPER emails an hour; the rest are listed in the next briefing.
...
- **H. BRIEFING 08:20**: market now, the 7 signal coins, next events, agent plan today, strategy progress, do / don't. **I. 14:20 / 21:20**: only what changed since the previous briefing; nothing changed = no email.
